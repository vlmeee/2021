...
clc
%% input
data=[2013 12 25 20 00 00];%% year, month, day, hour, minute,second
out0=[25498.66;0.00456;136.5;63.9;99.7;0];% a(km),e,RA(degrees),incl(degrees),w(degrees),TA(degrees)
step=600; %%seconds
day=7; %% day
%%
%mjuliandate() встроенная функция на случай, если будет необходимость
%использовать polarMotion()
%MJD=mjuliandate(data); JD = MJD + 2400000.5; 
...
for it=ittt
    ...
    keps=S*180/pi;
    %dxy=polarMotion(it); %встроенная функция. Данные берутся с IERS.com
    dxy=polarMotionCustom(it);
    %dxy_buffer(ni, :) = dxy;
    dx=dxy(1)
    dy=dxy(2);
...
subplot(2,3,4)
plot(itt,kepler_RAAN,'k','LineWidth',1)
grid on
hold on
ylabel('Градус');
xlabel('День');
...
subplot(2,3,5)
plot(itt,kepler_w,'r','LineWidth',1)
grid on
hold on
ylabel('Градус');
xlabel('День');
